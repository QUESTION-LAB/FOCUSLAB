...
새로고침해도 상태가 유지됩니다 (LocalStorage 기반).

---

## 2️⃣ Data Intelligence
...
집중 점수는 고정 공식이 아닙니다.
상관계수 기반으로 가중치가 조정됩니다.

FOCUSLAB은 정적인 점수 시스템이 아니라,
**사용자 데이터를 반영하는 적응형 구조**입니다.
...
FOCUSLAB은 UI 중심이 아니라,

**AI 모델을 중심으로 데이터가 흐르는 구조**로 설계되었습니다.

집중 점수 → 학습 → 예측 → 전략 → 피드백이
단절되지 않고 하나의 파이프라인으로 연결됩니다.
...
FOCUSLAB의 핵심은 기능이 아니라 **데이터 흐름 설계**입니다.
...
FOCUSLAB은 행동 데이터를 기반으로 개인의 집중 패턴을 학습하고 예측하는
**브라우저 기반 Neural Performance System**입니다.

단순 생산성 앱이 아니라,

**데이터 → 학습 → 예측 → 전략 → 강화**

로 이어지는 완전한 인지 퍼포먼스 루프를 설계합니다.
...
모델은 analytics 페이지에서 학습되며,
학습된 가중치는 index 페이지에서 실시간 예측에 사용됩니다.

즉,

> 학습 엔진과 실시간 퍼포먼스 시스템이 하나의 모델로 통합되어 있습니다.
